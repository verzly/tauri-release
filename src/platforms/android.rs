...
pub fn build(ctx: &BuildContext) -> Result<()> {
    ...
    let command = ctx.config.android.command.clone().unwrap_or_else(|| {
        let mut parts = vec![ctx.metadata.package_manager.tauri_command().to_string(), "android".into(), "build".into(), "--ci".into()];
