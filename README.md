...
```sh
cargo install --git https://github.com/verzly/tauri-release --tag v0.1.19 --force
```
...
```text
tauri-release-v0.1.19-<rust-host-target>
tauri-release-v0.1.19-<rust-host-target>.exe
```
...
```sh
curl -L -o tauri-release https://github.com/verzly/tauri-release/releases/latest/download/tauri-release-v0.1.19-x86_64-unknown-linux-gnu
...
```powershell
Invoke-WebRequest -Uri "https://github.com/verzly/tauri-release/releases/latest/download/tauri-release-v0.1.19-x86_64-pc-windows-msvc.exe" -OutFile "tauri-release.exe"
...
Release descriptions are generated by GitHub during the same workflow using the `What's Changed` style release notes from merged pull requests and commits. `CHANGELOG.md` is no longer maintained manually.
...
Release tags are managed by the manual GitHub Actions `Release` workflow. Start the workflow with a version such as `0.1.19`; it must match `Cargo.toml` `package.version`. After every standalone executable build succeeds, the workflow creates the immutable `vX.Y.Z` tag when it does not exist, creates or updates the GitHub Release with generated release notes and assets, then moves the `latest`, `vX.Y`, and `vX` channel tags to the same commit. Regular `master` pushes and pre-created GitHub Releases do not update release tags.
