...
```text
AppImage, deb, rpm, exe, msi, zip, dmg, apk, aab, ipa, sig
```

Special release metadata files such as `latest.json`, `release.json`, and `release-manifest.json` are also kept. Generic `.json` files are not collected by default, so project files such as `package.json`, `tauri.conf.json`, and `tsconfig.json` are not accidentally published. Everything else is treated as temporary build output.
...
Install stable releases directly from the Git repository. The recommended target is the moving `latest` tag, which is maintained by the manual release workflow. The same workflow also moves the matching major and minor channel tags, such as `v0` and `v0.2`.
...
```sh
cargo install --git https://github.com/verzly/tauri-release --tag v0.2 --force
...
```sh
cargo install --git https://github.com/verzly/tauri-release --tag v0.2.2 --force
```
...
```text
tauri-release-v0.2.2-<rust-host-target>
tauri-release-v0.2.2-<rust-host-target>.exe
```
...
```sh
curl -L -o tauri-release https://github.com/verzly/tauri-release/releases/latest/download/tauri-release-v0.2.2-x86_64-unknown-linux-gnu
...
```powershell
Invoke-WebRequest -Uri "https://github.com/verzly/tauri-release/releases/latest/download/tauri-release-v0.2.2-x86_64-pc-windows-msvc.exe" -OutFile "tauri-release.exe"
...
The manual `Release` workflow is built with `cargo-release`. Release `cargo-release` first when publishing both tools from a clean state. If `verzly/cargo-release` is private, add a `CARGO_RELEASE_TOKEN` repository secret with read access to that repository. The GitHub Release is created only after every standalone build succeeds, and assets are uploaded immediately after the release is created. Users do not have to wait for executables to be built after publishing a release.
...
- name: Build Tauri release artifacts
  uses: verzly/tauri-release@v0.2
  with:
...
Artifacts are collected by extension and explicit file names. Every selected platform must produce at least one artifact unless `allow_empty = true` is set intentionally:

```toml
[artifacts]
include_extensions = ["AppImage", "deb", "rpm", "exe", "msi", "zip", "dmg", "apk", "aab", "ipa", "sig"]
include_files = ["latest.json", "release.json", "release-manifest.json"]
keep_relative_paths = false
allow_empty = false
```

When `keep_relative_paths = false`, artifact file names must be unique inside each platform output directory. A collision fails the build instead of silently overwriting an asset. Set `keep_relative_paths = true` when two release files can share the same basename.
