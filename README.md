...
The manual `Release` workflow is built with `cargo-release`. Release `cargo-release` first when publishing both tools from a clean state. If `verzly/cargo-release` is private, add a `CARGO_RELEASE_TOKEN` repository secret with read access to that repository. The GitHub Release is created as a draft only after every standalone build succeeds, assets are uploaded while it is still hidden, and the workflow publishes it after the upload succeeds. Users do not have to wait for executables after publishing a release.
